...
def create_app() -> Flask:
    ...
    REQUEST_LIMIT_SETTINGS = config.get("request_limit", {"enabled": False, "limits": None})
    # Set up logging
    ...
    if CORS_SETTINGS["enabled"]:
        ...
        cors = CORS(app, resources={r"/placeholder/*": {
            "origins": CORS_SETTINGS["allowed_origins"]}
        })
        logger.info("CORS configuration applied.")
    else:
        logger.warning("CORS has been disabled. People may not be able to use your service to the fullest!")
    ...
    if REQUEST_LIMIT_SETTINGS["enabled"]:
        ...
        logger.warning("You have not set up a request limit. This is highly recommended for a production application!")
    # Import blueprints
