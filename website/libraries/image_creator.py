...
class PlaceholderImageCreator:
    ...
    def __init__(self, image_font:str, use_cache:bool, cache_directory:Optional[str]=None, cache_format:Optional[str]=None):
    ...
    def calculate_font(self, image:Image, image_draw, image_pil, font_name:str)->ImageFont.FreeTypeFont:
    ...
    def format_image_filepath(self, image:Image)->str:
        ...
        for parameter_key, parameter_value in format_parameters.items():
            ...
            output_filepath = output_filepath.replace(format_entry_text, str(parameter_value))
    ...
    def draw_image(self, image:Image)->str:
        ...
        image_pil = Image.new(mode="RGB", size=(image.width, image.height),
        ...
        # Add font text
        image_draw.text(text=image.text,
                        font=image_font,
                        fill=text_color_rgb,
        ...
        image_filepath = os.path.join(self.cache_directory,
                                      image_filename)
    ...
    def create_image(self, image:Image)->str:
        ...
        image_filepath = self.format_image_filepath(image) # Get where the image would be stored if it is
        if not self.use_cache or image_filepath not in os.listdir(self.cache_directory):
            self.logger.info("Image not previously created or cache disabled. Creating file...")
