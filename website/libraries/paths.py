...
CONFIGURATION_FILE_PATH = os.path.join(WORKING_DIR, "config.json5") # Location of the config file
IMAGES_DIRECTORY_PATH = os.getenv("PLACEHOLDY_IMAGES_DIRECTORY", os.path.join(WORKING_DIR, "images")) # Location of the image directory
# Check for uncreated config file
if not os.path.exists(CONFIGURATION_FILE_PATH):
    raise FileNotFoundError(f"Can not find configuration file. (Make sure that it exists on {CONFIGURATION_FILE_PATH}).")
